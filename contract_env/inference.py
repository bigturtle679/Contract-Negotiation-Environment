...
def _branch_sequence(tier: str) -> list[str]:
    if tier == "HIGH":
        return ["FLAG", "REJECT", "PROPOSE_COUNTER", "EDIT_CLAUSE"]
    if tier == "MODERATE":
        return ["FLAG", "EDIT_CLAUSE", "PROPOSE_COUNTER", "REJECT"]
    return ["FLAG", "ACCEPT", "EDIT_CLAUSE"]
...
def _maybe_llm_content(task: NegotiationTask, contract: str, action: Action) -> Action:
    ...
    try:
        ...
        user = (
            ...
            "Output only improved legal/contract language or negotiation text, no preamble."
        )
        resp = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You rewrite contract clauses and negotiation messages. Output only the text.",
                },
...
def _build_action(
    ...
    base = Action(action_type=t, content=_default_content(task, t) if t != "ACCEPT" else None)
...
def _choose_best(
    ...
    first, second, _, _ = (a1, a2, s1, s2) if s1 >= s2 else (a2, a1, s2, s1)
...
def run_episode() -> None:
    ...
    model_label = HF_MODEL if has_hf else "heuristic"
