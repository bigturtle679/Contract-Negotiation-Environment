...
def run_episode_direct(env: ContractEnv, initial_obs: dict[str, Any]) -> bool:
    ...
    while not done and it < max_iters and step_counter <= 20:
        ...
        _print_step(step_counter, action, reward_score, done, err)

    score = min(max(sum(rewards) / max(len(rewards), 1), 0.0), 1.0)
    success = bool(done and score >= 0.5)
    rlist = ",".join(f"{x:.2f}" for x in rewards)
    print(f"[END] success={str(success).lower()} steps={step_counter} score={score:.2f} rewards={rlist}", flush=True)
    return success
